...
import { threePicks, type MatchPicks } from "@/lib/dailyReview";
...
export async function GET(req: NextRequest) {
  ...
  // Uz svaki meč i tri ključne igre (1/2, set, gemovi) — sa ✓/✗ kad je meč gotov.
  const withPicks: (EnrichedWorldMatch & { picks: MatchPicks | null })[] = enriched.map((m) => ({ ...m, picks: threePicks(m) }));
  ...
  const byTournament = new Map<string, { tournament: string; tier: WorldMatch["tier"]; category: string; matches: (EnrichedWorldMatch & { picks: MatchPicks | null })[] }>();
